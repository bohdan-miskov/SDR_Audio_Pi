...
logger = logging.getLogger("DroneDetector")

class Config:
    pass

class DroneDetector:
    def __init__(self, model_path='models/convn.keras', pickle_path='models/convn.p'):
    ...
    def predict(self, audio_chunk, rate=16000):
        ...
        for i in range(0, len(clean_audio) - self.config.step, self.config.step):
            ...
            denominator = self.config.max - self.config.min
            if denominator == 0:
                x = np.zeros_like(x)
            else:
                x = (x - self.config.min) / denominator
